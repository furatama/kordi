...
@section('content')
		<div class="my-3 clearfix">
            ...
            <!-- <a class="btn btn-lg btn-danger float-right ml-2" href="{!! route('cetak',['from'=>'relawan']) !!}">PDF</a> -->
            @if (Auth::user()->hasAccess('RLC'))
            <a class="btn btn-lg btn-primary float-right ml-2" href="{!! route('relawan.report',['by'=>$by,'id'=>$id]) !!}">Report</a>
            <a class="btn btn-lg btn-success float-right" href="{!! route('relawan.create') !!}">Tambah Data</a>
            @endif
		</div>
...
$(function() {
    var table = $('#main-table').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{!! isset($mtableref) ? $mtableref : route('relawan.fetch') !!}',
        aoColumns: [
            ...
            { 
            	...
            	render: function(data, type, row, meta) {
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                        <form action="{{route('relawan.index')}}/${row.id}" method="post" onsubmit="return confirm('Yakin ingin hapus data?');">
                          @csrf
                          @method('DELETE')
                            @if (Auth::user()->hasAccess('RLU'))
                            <button type="button" class="dropdown-item btn btn-info" id="main-edit" onclick="window.location = '{!! route('relawan.index') !!}/${row.id}/edit'">Edit</button>
                            @endif
